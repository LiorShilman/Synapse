...
export default function SimToolbar() {
  ...
  const handleSave = () => {
    ...
    // Consensus protocol patterns to filter out
    ...
    // Decision reasoning — each agent's most substantive thought (not protocol messages)
      ...
      return `
        <div class="decision-card" style="border-right-color:${meta.color}">
          <div class="decision-agent">
            ...
            <span style="color:#5A7A9A;font-size:0.8rem">${meta.role}</span>
          </div>
          <div style="color:#CCDDEE;font-size:0.95rem;line-height:1.7">${thought}</div>
        </div>`;
    }).filter(Boolean).join('');

    // Key synthesis/insight messages — deduplicated, filtered
    ...
    const keyInsights = store.messages
      ...
      .map((m) => {
        ...
        return `
          <div style="display:flex;gap:8px;align-items:flex-start;padding:8px 0;border-bottom:1px solid rgba(100,160,255,0.05)">
            <span>${icon}</span>
            <span style="color:${color};font-weight:700;white-space:nowrap;font-size:0.9rem">${fromName}:</span>
            <span style="color:#AABBCC;font-size:0.9rem;line-height:1.6">${m.text}</span>
          </div>`;
...
<title>סינאפס — סימולציה: ${store.currentProblem}</title>
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .brand { font-size: 1rem; letter-spacing: 6px; text-transform: uppercase; color: #4FC3F7; margin-bottom: 12px; font-weight: 300; }
  .title { font-size: 2.2rem; font-weight: 900; background: linear-gradient(135deg, #4FC3F7, #CE93D8, #FFAB40); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; margin-bottom: 8px; }
  .subtitle { font-size: 1rem; color: #4A6A8A; margin-bottom: 20px; }
  .problem-box { background: linear-gradient(135deg, rgba(79,195,247,0.1), rgba(206,147,216,0.1)); border: 1px solid rgba(79,195,247,0.2); border-radius: 16px; padding: 24px 32px; font-size: 1.3rem; font-weight: 500; max-width: 700px; margin: 0 auto 24px; color: #fff; }
  .meta-row { display: flex; justify-content: center; gap: 32px; flex-wrap: wrap; }
  .meta-item { display: flex; align-items: center; gap: 8px; color: #8899AA; font-size: 0.95rem; }
  .meta-val { color: #E0E6ED; font-weight: 700; }
  .section { margin-top: 48px; }
  .section-title { font-size: 1.3rem; font-weight: 700; margin-bottom: 20px; padding-bottom: 10px; border-bottom: 1px solid rgba(79,195,247,0.15); display: flex; align-items: center; gap: 10px; }
  .section-icon { font-size: 1.4rem; }
  .agents-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 16px; }
  .agent-card { background: rgba(10, 22, 40, 0.8); border: 1px solid rgba(100,160,255,0.1); border-radius: 12px; padding: 18px 20px; }
  ...
  .conf-bar { height: 6px; background: rgba(255,255,255,0.06); border-radius: 3px; margin-bottom: 10px; overflow: hidden; }
  .conf-fill { height: 100%; border-radius: 3px; }
  .agent-thought { font-size: 0.9rem; color: #AABBCC; font-style: italic; }
  .timeline { display: flex; flex-direction: column; gap: 6px; }
  .msg { background: rgba(10, 22, 40, 0.6); border-right: 3px solid rgba(100,160,255,0.15); border-radius: 0 8px 8px 0; padding: 12px 18px; }
  .msg-user { background: rgba(79,195,247,0.06); border-right-color: #81D4FA; }
  .msg-synthesis { background: rgba(255,213,79,0.06); border-right-color: #FFD54F; }
  ...
  .consensus-event { background: rgba(102,187,106,0.08); border-radius: 8px; padding: 10px 16px; margin-bottom: 8px; display: flex; gap: 12px; align-items: center; }
...
<div class="page">
  <div class="header">
    ...
    <h1 class="title">דוח סימולציה</h1>
    <div class="subtitle">מצב סימולציה — מחשבות מבוססות תבניות</div>
  ...
  <div class="section">
    <div class="section-title"><span class="section-icon">🤖</span> מצב סוכנים</div>
  ...
  <div class="footer">
    ...
    a.href = url;
    a.download = `synapse-sim-${Date.now()}.html`;
    a.click();
